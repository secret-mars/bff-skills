...
function timeAgo(blockTimeMs: number): string {
...
async function gainers(limit: number): Promise<void> {
  const clampedLimit = Math.max(1, Math.min(25, limit));
  const url = ENDPOINTS.topGainers(clampedLimit);

  try {
    const rows = await fetchTenero<TokenGainer[]>(url);
    const items = Array.isArray(rows) ? rows : [];

    const formatted: FormattedGainer[] = items.map((t) => ({
      symbol: t.symbol ?? "?",
      name: t.name ?? "?",
  ...
  switch (command) {
    ...
    default:
      out({
        ...
        data: { availableCommands: ["doctor", "scan", "gainers"] },
        error: `Unknown command: "${command ?? ""}". Use: doctor | scan [--limit N] | gainers [--limit N]`,
      });
      process.exit(1);
  }
}
